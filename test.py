...
def evaluate_model(
    ...
    for batch in tqdm.tqdm(dataloader, desc="Evaluating"):
        ...
        elif model_dtype == "float32" or model_dtype == 32:
            image_tensors = image_tensors.float()
        else:
            # Default to float32 for CPU
            image_tensors = image_tensors.float()
            print(f"Warning: Unknown model dtype {model_dtype}, using float32")


        # Move data to device (CPU or CUDA)
        ...
        if pointer_args is not None:
            pointer_args["coord_input_idx"] = pointer_args["coord_input_idx"].to(device)
            pointer_args["coord_input_length"] = pointer_args["coord_input_length"].to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_path", type=str, default=None, help="Optional Lightning checkpoint path")
    ...
    parser.add_argument("--max_samples", type=int, default=-1, help="Limit number of samples for testing")
   ...
   # Always load from pytorch_model.bin
    bin_path = os.path.join(args.model_name_or_path, "pytorch_model.bin")
    print(f"Loading model weights from: {bin_path}")
    saved_state_dict = torch.load(bin_path, map_location=torch.device("cpu"))
    encoder_state_dict = {
        k[len("encoder.") :]: v
    ...
    torch_dtype = getattr(model_config, 'torch_dtype', 'float32')
    if torch_dtype == "float16":
        model.half()
    elif torch_dtype == "bfloat16":
        model.bfloat16()
    else:
        # Default to float32 for CPU training
        model.float()


    # Set device - prefer CPU for this setup
    if torch.cuda.is_available() and torch_dtype in ["float16", "bfloat16"]:
        model = model.cuda()
    ...
    print(f"Dataset & loader setup complete. Evaluating {len(dataset)} samples...")
    import time
    ...
    print(f"Total samples evaluated: {len(result_collection)}")
    
    # Print summary statistics
    if result_collection:
        edit_distances = [result["edit_distance"] for result in result_collection.values()]
