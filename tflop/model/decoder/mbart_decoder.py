...
class MBARTDecoder(nn.Module):
    ...
    def get_tag2coord_ptr_loss(
        ...
        total_bbox_token_length: int,
        input_coords_length: torch.Tensor,
        pointer_label: torch.Tensor,
        pointer_mask_label: torch.Tensor,
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Calculate tag-to-coordinate pointer loss with safe mask indexing.
        """
        batch_size = output_seq.size(0)

        # 1. Compute combined feature (bsz, T_text, T_bbox)
        key_feat = self.k_linear(output_seq[:, :total_bbox_token_length])
        query_feat = self.q_linear(output_seq[:, total_bbox_token_length + 1 :])
        norm_key = F.normalize(key_feat, dim=-1)
        norm_query = F.normalize(query_feat, dim=-1)
        data_combined_feat = torch.bmm(norm_query, norm_key.transpose(1, 2))

        # 2. Ensure pointer_label dtype matches
        if pointer_label.dtype != query_feat.dtype:
            pointer_label = pointer_label.to(query_feat.dtype)

        is_empty_loss = 0.0
        is_not_empty_loss = 0.0
        batchwise_acc = []

        for i in range(batch_size):
            feat = data_combined_feat[i]           # (T_text, T_bbox)
            mask = pointer_mask_label[i]           # (T_text,)
            label = pointer_label[i]               # (T_text, T_bbox)

            # Align lengths
            min_len = min(feat.size(0), mask.size(0), label.size(0))
            feat = feat[:min_len]
            mask = mask[:min_len]
            label = label[:min_len]
            ...
            # Skip empty mask
            if mask.sum() == 0:
                batchwise_acc.append(feat.new_tensor(0.0))
                continue

            # Safe indexing
            selected_feat = feat[mask]
            selected_label = label[mask]
            if selected_feat.numel() == 0 or selected_label.numel() == 0:
                batchwise_acc.append(feat.new_tensor(0.0))
                continue

            # Empty-cell loss on index 0
            empty_pred = selected_feat[:, 0]
            empty_label = selected_label[:, 0]
            is_empty_loss += F.binary_cross_entropy_with_logits(empty_pred, empty_label)

            # Non-empty loss across valid coords
            non_empty_pred = selected_feat[:, 1 : (input_coords_length[i] + 1)] / 0.1
            non_empty_label = selected_label[:, 1 : (input_coords_length[i] + 1)]
            valid_cols = non_empty_label.sum(0) == 1
            if valid_cols.sum() == 0:
                batchwise_acc.append(feat.new_tensor(0.0))
                continue

            pred_t = non_empty_pred.transpose(0, 1)[valid_cols]
            label_t = non_empty_label.transpose(0, 1)[valid_cols]
            is_not_empty_loss += F.cross_entropy(pred_t, label_t.argmax(dim=-1))

            # Accuracy
            with torch.no_grad():
                preds = torch.argmax(non_empty_pred, dim=0)
                golds = label_t.argmax(dim=-1)
                acc = (preds == golds).float().mean()
                batchwise_acc.append(acc)

        # Finalize
        if batchwise_acc:
            pointing_acc = torch.stack(batchwise_acc).mean()
        else:
            pointing_acc = output_seq.new_tensor(0.0)

        is_empty_loss /= batch_size
        is_not_empty_loss /= batch_size
