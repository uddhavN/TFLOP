...
class TFLOP(PreTrainedModel):
    ...
    def get_dr_point_pred(self, last_hidden_state_collection, decoder_output_sequences):
        ...
        print(f"bbox_token_cnt in config: {self.config.bbox_token_cnt}")
        print(f"last_hidden_state_collection shape: {last_hidden_state_collection.shape}")
        print(f"decoder_output_sequences shape: {decoder_output_sequences.shape}")

        # Compute actual lengths for text tokens and query tokens
        total_seq_len = last_hidden_state_collection.shape[1]
        bbox_cnt = self.config.bbox_token_cnt
        # text_token_count = decoder_output_sequences.shape[1]
        text_len = decoder_output_sequences.shape[1] - 2      # (bsz, text_token_cnt-2)
        query_len = total_seq_len - (1 + bbox_cnt)           # (bsz, text_token_cnt-2)
        min_len = min(text_len, query_len)

        # Slice hidden states and decoder outputs to the same min_len
        query_feature = self.decoder.q_linear(
            last_hidden_state_collection[:, (1 + bbox_cnt):(1 + bbox_cnt + min_len)]
        )  # (bsz, min_len, d_model)

        key_feature = self.decoder.k_linear(
            last_hidden_state_collection[:, :bbox_cnt]
        )  # (bsz, bbox_cnt, d_model)
        ...
        for i in range(0, batch_size, iter_size):
            norm_q = torch.nn.functional.normalize(query_feature[i : i + iter_size], dim=-1)
            norm_k = torch.nn.functional.normalize(key_feature[i : i + iter_size], dim=-1)
            tmp = torch.bmm(norm_q, norm_k.transpose(1, 2))  # (sub-bsz, min_len, bbox_cnt)
            combined_feature.append(tmp)
        combined_feature = torch.cat(combined_feature, dim=0)  # (bsz, min_len, bbox_cnt)

        # Prepare coordinate logits and mask only on valid text tokens
        coord_logits = combined_feature[:, :, 1:]               # (bsz, min_len, bbox_cnt-1)
        text_token_seq = decoder_output_sequences[:, 2 : 2 + min_len]  # (bsz, min_len)
        is_data_tensor = torch.zeros_like(text_token_seq, dtype=torch.bool)
        for data_id in self.data_ids:
            is_data_tensor |= (text_token_seq == data_id)       # (bsz, min_len)

        # Apply mask safely (shapes now aligned)
        coord_logits[~is_data_tensor] = float("-inf")          # (bsz, min_len, bbox_cnt-1)
        coord_one_hot = (
            torch.nn.functional.one_hot(
                torch.argmax(coord_logits, dim=-1),
                num_classes=coord_logits.shape[-1],
            )
            .to(coord_logits.dtype)
        )  # (bsz, min_len, bbox_cnt-1)

        # Prepend empty flag and return
        is_empty = (coord_one_hot.sum(dim=-1) == 0).unsqueeze(-1).to(coord_one_hot.dtype)
        combined_feature = torch.cat([is_empty, coord_one_hot], dim=-1)  # (bsz, min_len, bbox_cnt)
