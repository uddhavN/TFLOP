...
class TFLOPModelPLModule(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        for pred, answer in zip(pred_collection, answers):
            ...
            denom = max(len(pred_string), len(answer_string))
            if denom == 0:
            # Avoid division by zero, assign a default score (e.g. 0.0 or 1.0)
                normalized_distance = 0.0  # or consider float('inf') if appropriate
            else:
                normalized_distance = distance(pred_string, answer_string) / denom
            score_set.append(normalized_distance)
    ...
    def configure_optimizers(self):
        ...
        max_iter = self.config.training.get("max_steps", 10000)
    ...
    def on_save_checkpoint(self, checkpoint):
        """Save model and tokenizer with JSON-serializable config handling"""
        import json
        ...
        # Handle non-JSON serializable objects in model config
        if hasattr(self.model, 'config') and hasattr(self.model.config, '__dict__'):
            ...
            # Convert each config item to JSON-serializable format
            for key, value in original_config.items():
                try:
                    ...
                    if hasattr(value, '_content'):  # OmegaConf ListConfig/DictConfig
                        serializable_config[key] = OmegaConf.to_container(value, resolve=True)
                    elif hasattr(value, '__dict__'):  # Custom objects
                        serializable_config[key] = str(value)
                    else:
                        serializable_config[key] = str(value)
            
            # Temporarily replace config with serializable version
            self.model.config.__dict__.clear()
            self.model.config.__dict__.update(serializable_config)
            ...
            # Fallback: save without config conversion
            try:
                self.model.save_pretrained(save_path)
                if hasattr(self.model, 'decoder') and hasattr(self.model.decoder, 'tokenizer'):
                    self.model.decoder.tokenizer.save_pretrained(save_path)
