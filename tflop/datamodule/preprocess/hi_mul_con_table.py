...
def get_rowwise_HiMulConET_Coeff(
    ...
    """
    Function to get rowwise HiMulConET Coefficient Matrix with robust error handling
    """
    ...
    # Get table breakdown with error handling
    table_breakdown = breakdown_otsl_seq(sliced_input_ids.tolist(), tokenizer)
    
    # **CRITICAL FIX: Handle empty table breakdown**
    if not table_breakdown or len(table_breakdown) == 0:
        print(f"Warning: Empty table_breakdown from breakdown_otsl_seq()")
        print(f"Input IDs length: {len(sliced_input_ids)}")
        print(f"Sample input IDs: {sliced_input_ids[:min(10, len(sliced_input_ids))].tolist()}")
        # Return identity matrix or zeros as appropriate default
        match_index = torch.eye(bbox_token_cnt)
        return match_index
    
    # **ADDITIONAL FIX: Check if first row exists and is not empty**
    if len(table_breakdown[0]) == 0:
        print(f"Warning: First row of table_breakdown is empty")
        print(f"Table breakdown structure: {[len(row) for row in table_breakdown]}")
        match_index = torch.eye(bbox_token_cnt)
        return match_index

    table_data_idx = (is_data_tensor).nonzero(as_tuple=True)[0]
    
    # **SAFETY CHECK: Validate table structure before processing**
    try:
        ...
        for data_token_id in table_data_idx:
            # **BOUNDARY CHECK**: Prevent index out of range during traversal
            while (table_breakdown_row_id < len(table_breakdown) and 
                   table_breakdown_col_id < len(table_breakdown[table_breakdown_row_id]) and
                   table_breakdown[table_breakdown_row_id][table_breakdown_col_id] is None):
                ...
                if table_breakdown_col_id >= len(table_breakdown[table_breakdown_row_id]):
                    ...
                    # **EXIT CONDITION**: Prevent infinite loops
                    if table_breakdown_row_id >= len(table_breakdown):
                        break

            # **BOUNDARY CHECK**: Ensure we haven't exceeded table bounds
            if (table_breakdown_row_id >= len(table_breakdown)):
                print(f"Warning: Exceeded table bounds during processing")
                break
            ...
            # **SAFETY CHECK**: Ensure the cell exists before appending
            if (table_breakdown_row_id < len(table_breakdown) and
                table_breakdown_col_id < len(table_breakdown[table_breakdown_row_id])):
                if table_breakdown[table_breakdown_row_id][table_breakdown_col_id] is not None:
        ...
        print(f"Table breakdown structure: {[len(row) if row else 0 for row in table_breakdown]}")
        match_index = torch.eye(bbox_token_cnt)
        return match_index
    
    # **SAFE COLUMN COUNT**: This line was causing the original error
    column_count = len(table_breakdown[0]) if table_breakdown and table_breakdown[0] else 0
    
    if column_count == 0:
        print(f"Warning: Zero column count detected")
        match_index = torch.eye(bbox_token_cnt)
        return match_index
    
    # **FIX: Handle incomplete last row safely**
    if len(table_breakdown) > 0 and len(table_breakdown[-1]) < column_count:
        table_breakdown[-1].extend(
            [None for _ in range(column_count - len(table_breakdown[-1]))]
        )

    # Continue with rest of the original function...
        ...
        for table_col_id, table_cell in enumerate(table_row):
            ...
            if table_cell[0] > 1:  # this is rowspan
                ...
                for rowspan_id in range(rowspan_cnt):
                    if table_row_id + rowspan_id >= len(table_breakdown):
...
def get_columnwise_HiMulConET_Coeff(
    ...
    # **CRITICAL FIX: Handle empty table breakdown**
    if not table_breakdown or len(table_breakdown) == 0:
        print(f"Warning: Empty table_breakdown from breakdown_otsl_seq() in columnwise function")
        print(f"Input IDs length: {len(sliced_input_ids)}")
        print(f"Sample input IDs: {sliced_input_ids[:min(10, len(sliced_input_ids))].tolist()}")
        # Return identity matrix or zeros as appropriate default
        match_index = torch.eye(bbox_token_cnt)
    ...
    # **ADDITIONAL FIX: Check if first row exists and is not empty**
    if len(table_breakdown[0]) == 0:
        print(f"Warning: First row of table_breakdown is empty in columnwise function")
        print(f"Table breakdown structure: {[len(row) for row in table_breakdown]}")
        match_index = torch.eye(bbox_token_cnt)
        return match_index
    ...
    try:
        ...
        # (Similar to the rowwise function, but for columns)
        
        # **SAFE COLUMN COUNT**: This line was causing the original error
        column_count = len(table_breakdown[0]) if table_breakdown and table_breakdown[0] else 0
        
        if column_count == 0:
            print(f"Warning: Zero column count detected in columnwise function")
            match_index = torch.eye(bbox_token_cnt)
            return match_index
        
        # Continue with the rest of your columnwise processing...
        # [Rest of the original function logic]
        
    except Exception as e:
        print(f"Error during columnwise table processing: {e}")
        print(f"Table breakdown structure: {[len(row) if row else 0 for row in table_breakdown]}")
        match_index = torch.eye(bbox_token_cnt)
        return match_index
