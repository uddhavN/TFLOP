...
def convert_gold_coords(gold_coords: List[str]):
    ...
    def is_bbox_invalid(bbox_coord_value):
        ...
        return abs(x1 - x2) < 1e-5 or abs(y1 - y2) < 1e-5

    for coord in gold_coords:
        # Assuming coord is like [x1, y1, x2, y2, fill_flag, *tokens]
        four_coord_values = [float(x) for x in coord[:4]]
        gold_coord_dict["coords"].append(four_coord_values)

        # fill_flag is index 4, tokens are from 5 onward
        fill_flag = int(coord[4]) if len(coord) > 4 else 0
        gold_coord_dict["isFilled"].append(False if is_bbox_invalid(four_coord_values) else fill_flag == 2)
        text_list = coord[5:] if len(coord) > 5 else []
        gold_coord_dict["text"].append(" ".join(map(str, text_list)))
